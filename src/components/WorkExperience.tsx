...
const WorkExperience = () => {
  ...
  return (
    <section id="experience" ref={sectionRef} className="py-20 md:py-28 bg-black text-white">
      <div className="container-custom">
        <div className="text-center max-w-3xl mx-auto mb-16 reveal-on-scroll">
          <span className="badge bg-white/20 text-white mb-3">Work Experience</span>
          <h2 className="section-title mb-6">Professional Journey</h2>
          <p className="text-gray-400">Building innovative design solutions across various industries</p>
        </div>
          ...
          {experiences.map((exp, index) => (
            <div key={index} className="relative">
              {index < experiences.length - 1 && (
                <div className="absolute left-[28px] top-[70px] bottom-0 w-0.5 h-[calc(100%_-_50px)] bg-white/10"></div>
              )}
              
              <div className="grid md:grid-cols-12 gap-6 md:gap-10 items-start reveal-on-scroll">
                <div className="md:col-span-4 relative z-10">
                  <div className="bg-white/5 border border-white/10 text-white p-5 rounded-xl">
                ...
                {index < 2 && (
                  <div className="md:col-span-8 bg-white/5 border border-white/10 p-6 rounded-xl">
                    <div className="mb-4">
                      <h4 className="text-lg font-semibold text-white mb-2">Challenge:</h4>
                      <p className="text-gray-400">{exp.challenge}</p>
                    </div>
                    
                    <div className="mb-4">
                      <h4 className="text-lg font-semibold text-white mb-2">Solution:</h4>
                      <p className="text-gray-400">{exp.solution}</p>
                    </div>
                      ...
                      <ul className="space-y-2">
                        {exp.contributions?.map((contribution, i) => (
                          <li key={i} className="flex items-start">
                            <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-white/10 text-white mr-2 mt-0.5">
                              <Check size={12} />
                            </span>
                    ...
                    <div>
                      ...
                      <p className="text-gray-400">{exp.impact}</p>
                    </div>
                  </div>
                )}
                ...
                {index >= 2 && (
                  <div className="md:col-span-8 bg-white/5 border border-white/10 p-6 rounded-xl">
                    <div className="text-center py-4">
                      <p className="text-gray-400">View more details about my work at {exp.company}</p>
                    </div>
                  </div>
                )}
              </div>
